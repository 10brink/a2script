...
def fetch_weather_summary(d: date) -> str:
    ...
    return "Ann Arbor weather: " + ", ".join(parts) + "."
...
def subject_for_date(d: date) -> str:
    return f"Today in Ann Arbor: {d.strftime('%a %b %d')}"
...
def should_send_for_location() -> bool:
    if os.getenv("FORCE_SEND", "false").lower() == "true":
        return True
    return is_in_ann_arbor()
...
def main() -> None:
    ...
    if not email_enabled():
        print("Email disabled — email skipped.")
        return

    if not should_send_for_location():
        print("Not in Ann Arbor area — email skipped.")
        return
